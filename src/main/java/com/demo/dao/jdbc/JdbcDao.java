...
public class JdbcDao {
    ...
    public static String createPageSql(String sql, int page, int rows){
        ...
        sqlParam[1] = beginNum+"";
        sqlParam[2] = rows+"";
        if(dbType.toLowerCase().indexOf(DATABSE_TYPE_MYSQL)!=-1){
            sql = MessageFormat.format(MYSQL_SQL, sqlParam);
        }else if(dbType.toLowerCase().indexOf(DATABSE_TYPE_POSTGRE)!=-1){
            ...
            if(dbType.toLowerCase().indexOf(DATABSE_TYPE_ORACLE)!=-1) {
                sql = MessageFormat.format(ORACLE_SQL, sqlParam);
            } else if(dbType.toLowerCase().indexOf(DATABSE_TYPE_SQLSERVER)!=-1) {
